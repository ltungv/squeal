...
test "lru cache get and set" {
    ...
    // overwriting values
    try testing.expectEqual(@as(?u64, 420), try cache.set(0, 69));
    try testing.expectEqual(@as(?u64, 666), try cache.set(1, 420));
    try testing.expectEqual(@as(?u64, 69), try cache.set(2, 666));
}
...
test "lru cache invalidation order" {
    ...
    try testing.expectEqual(@as(?u64, 69), try cache.set(3, 777));
