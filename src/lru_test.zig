...
test "lru cache get and set" {
    ...
    defer cache.deinit();
    // set new
    try testing.expectEqual(@as(?u64, null), try cache.set(0, 69));
    try testing.expectEqual(@as(?u64, null), try cache.set(1, 420));
    try testing.expectEqual(@as(?u64, null), try cache.set(2, 666));
    // asserts
    ...
    // overwriting values
    try testing.expectEqual(@as(?u64, 69), try cache.set(0, 420));
    try testing.expectEqual(@as(?u64, 420), try cache.set(1, 666));
    try testing.expectEqual(@as(?u64, 666), try cache.set(2, 69));
...
test "lru cache invalidation order" {
    ...
    try testing.expectEqual(@as(?u64, null), try cache.set(2, 666));
    // invalidating values
    try testing.expectEqual(@as(?u64, null), try cache.set(3, 777));
    try testing.expectEqual(@as(?u64, null), try cache.set(4, 777));
    try testing.expectEqual(@as(?u64, null), try cache.set(5, 777));
    // asserts
